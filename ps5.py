...
from ps5_helpers import generate_complete_graph
...
def iset_bfs_3_coloring(G):
    size = math.ceil(G.N/3)
    for s in range(size):
        for S_ in combinations(range(G.N), size - s):
            S = list(S_)
            if is_independent_set(G, S):
                return bfs_2_coloring(G, S)
    return None
